...
fun Route.catMaleRoutes(service: CatMaleService, litterService: LitterService) {
    route("cat-males") {
        authenticate("auth-jwt") {
            ...
            get("{id}/litters") {
                val id = call.parameters["id"]?.toIntOrNull()
                    ?: throw IllegalArgumentException("Некорректный id")
                service.getById(id)
                call.respond(litterService.findByFatherId(id))
            }
            post {
                call.requireBreeder()
                val userId = call.principalUserId()
                val request = call.receive<SaveCatMaleRequest>()

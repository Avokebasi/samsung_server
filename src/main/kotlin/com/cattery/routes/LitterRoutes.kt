...
fun Route.litterRoutes(service: LitterService, kittenService: KittenService) {
    route("litters") {
        authenticate("auth-jwt") {
            ...
            get("{id}/kittens") {
                val id = call.parameters["id"]?.toIntOrNull()
                    ?: throw IllegalArgumentException("Некорректный id")
                call.respond(kittenService.listByLitter(id))
            }
            post {
                call.requireBreeder()
                val userId = call.principalUserId()
                val request = call.receive<SaveLitterRequest>()
